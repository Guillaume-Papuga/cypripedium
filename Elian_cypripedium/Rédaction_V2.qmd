...
Cette espèce présente un fort intérêt patrimonial. Par ses exigences écologiques marquées, notamment son besoin de milieux forestiers semi-ouverts, elle peut jouer un rôle d’espèce parapluie, contribuant indirectement à la conservation d’autres espèces végétales associées. En effet, elle nécessite souvent une gestion pour garder des structures forestières ouvertes et éviter l’embroussaillement.
Cette espèce est aussi réglementée par plusieurs statuts de protection lui conférant une protection face à la cueillette et à la destruction ou dégradation de son milieu :
...
-	Au niveau national en tant qu’espèce protégée en France (Arrêté du 20 janvier 1982).
Enfin, le Sabot de Vénus est présent sur la liste rouge nationale de 2019 en tant qu’espèce « Quasi-menacée » en accord avec son statut sur la liste rouge européenne de l’UICN.
Bien que majoritairement situé en contexte de plaine, le territoire du Parc national de forêts, avec son microclimat froid et humide et ses reliefs variés abrite plusieurs populations du Sabot de Vénus. Ce parc est situé à la frontière entre les départements de la Côte-d’Or dans la région Bourgogne-Franche-Comté et de la Haute-Marne dans la région Grand-Est. 
Son statut de parc national depuis 2019 lui confère le devoir de protéger la biodiversité notamment en développant la connaissance des espèces et leur suivi scientifique. C’est dans ce cadre qu’un suivi de Cypripedium calceolus est requis. Pour cela, une étude pilote a été réalisée en 2025 pour émettre des points de vigilance méthodologique quant à l’établissement d’un nouveau protocole de suivi robuste et standardisé à l’échelle du territoire du Parc national de forêts.
Cette étude a permis le suivi de 5 sites présents dans le Cœur du parc. Dans chaque site, des quadrats de 100 m² ont été placés en suivant un échantillonnage stratifié permettant de prendre en compte des différences de densité d’individus observées. Chaque quadrat a été prospecté sur trois sessions permettant de couvrir plusieurs stades phénologiques de l’espèce et de faire varier sa détectabilité :
...
Deux opérateurs ont réalisé le suivi en notant pour chaque individu le stade phénologique, la taille, le nombre de fleurs et de fruits et la présence de prédation. De plus, ils ont aussi évalué pour chaque quadrat le recouvrement de cinq strates végétales.
Enfin, lors de chaque session, entre 4 et 6 observateurs ont prospecté chaque quadrat pendant une durée contrainte de 10 à 15 minutes. Pour chaque individu observé, ils ont renseigné le stade phénologique. Cette approche permet d’évaluer à la fois les erreurs d’attribution des stades phénologiques et les taux de détection, en comparaison avec le nombre total d’individus recensés par les opérateurs. 
Un premier rapport auquel il faut se référer est celui de Léa DESCLOS en 2025 qui permet d’aborder le protocole plus en profondeur et qui donne déjà des clés sur les résultats de cette étude pilote.
...
Pour éviter la colinéarité entre les stades phénologiques et la taille, deux modèles sont effectués en mettant à chaque fois une des deux variables.
Les modèles sont des GLMM binomiaux avec comme variable réponse détection. Les variables explicatives fixes sont :
- le site, la session et leur interaction : On s'attend à ce que la détectabilité augmente avec le temps, quelle varie potentiellement entre sites et que cet détectabilité entre sites change au cours du temps
- la phénologie observée par les opérateurs : On s'attend à ce que le stade juvénile soit détectable moins facilement et à des erreurs entre fleurs fannées et non fleuri
- les strates végétales : on s'attend à ce que la détectabilité soit moins bonnes dans des zones denses
- le risque d'erreur basé sur l'autonotation des observateurs : on s'attend à ce que la détectabilité diminue avec le risque d'erreur
Les variables explicatives aléatoires sont :
- l'individu emboité dans quadra (déjà intégré dans notre écriture de l'individu)
- le quadra emboité dans site (déjà intégré dans notre écriture du quadrat)
- l'observateur

```{r, echo=FALSE}
#Modèle stade phénologique
glmm_final_pheno <- glmer(
  detection ~ site * session + phenologie_verif + S_muscinale + S_herbacee + S_arbustive_basse + S_arbustive_haute + S_arborescente + risque_erreur + (1|observateur) + (1|ID_ind) + (1|ID_quadrat),
  data = tab_complet,
  family = binomial,
...
glmm_final_taille <- glmer(
  detection ~ site * session + taille_sc + S_muscinale + S_herbacee + S_arbustive_basse + S_arbustive_haute + S_arborescente + risque_erreur + (1|observateur) + (1|ID_ind) + (1|ID_quadrat),
  data = tab_complet,
  family = binomial,
  control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))
)

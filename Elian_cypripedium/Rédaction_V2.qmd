...
tab_sess <- tab_complet_final %>%
  ...
  summarise(
    detection = ifelse(
      all(is.na(detection)),
      NA_integer_,
      max(as.integer(detection), na.rm = TRUE)
    ),
    phenologie = {
    ...
    recouvrement = if ("recouvrement_PC1" %in% names(cur_data()))
      mean(recouvrement_PC1, na.rm = TRUE)
      else NA_real_,
    site = first(site),
    .groups = "drop"
  )
...
processed <- process.data(histories_for_mark, model = "Multistrata", groups = c("site"))
...
#TEST CRDMS 
# Ajouter 0 au début et à la fin pour CRDMS
time.intervals_crdms <- c(0, rep(1, T - 1), 0)

processed_crdms <- process.data(histories_rmark,
                                model = "CRDMS",
                                time.intervals = time.intervals_crdms,
                                strata.labels = states)
...
# Modèle 1 : base parcimonieuse (S constant, p selon état, Psi constant)
m1_crdms <- mark(processed_crdms, ddl_crdms, model.parameters = list(
  S   = list(formula = ~1),
  p   = list(formula = ~ stratum),
  Psi = list(formula = ~1)
))

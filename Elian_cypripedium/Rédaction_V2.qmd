...
- les quadrat emboités dans les sites : un même quadrat étant mesuré à trois intervalles il faut enlever cette autocorrélation temporelle
...
Les modèles sont des GLMM binomiaux avec comme variable réponse détection. 
Les variables explicatives fixes sont : 
- le site : On s'attend à ce que la détectabilité varie entre sites
- la session : On s'attend à ce que la détectabilité augmente avec le temps
- l'interaction site et session : on s'attend à ce que la détection dans les sites ne réagisse pas de la même manière au cours du temps
...
- l'interaction site et phénologie : on s'attend à ce la détection dans les sites ne soit pas la même en fonction des stades phénologiques
...
glmm_final_pheno <- glmer(
  detection ~ site + session + phenologie_verif + site:session + S_herbacee + S_arbustive_basse + S_arbustive_haute + S_arborescente + risque_erreur + (1|observateur) + (1|site:quadrat:marquage) + (1|site:quadrat),
  data = tab_complet,
  family = binomial,
...
glmm_final_taille <- glmer(
  detection ~ site + session + taille_sc + site:session + S_herbacee + S_arbustive_basse + S_arbustive_haute + S_arborescente + risque_erreur + (1|observateur) + (1|site:quadrat:marquage) + (1|site:quadrat),
  data = tab_complet,
  family = binomial,
  control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))
)
...
#Les variables fixes du modèle expliquent 25% de la variance de la détection. En incluant les effets aléatoire la part de variance expliquée monte à 56% ce qui montre une forte influence des effets aléatoires.
...
# -------------------------------
phenology_codes <- c("j" = 1, "nf" = 2, "f" = 3, "ff" = 4)
...
# -------------------------------
# 1. Préparer les données phénologiques (sessions 2 et 3)
# -------------------------------
phenology_codes <- c("j" = 1, "nf" = 2, "f" = 3, "ff" = 4, "F" = 5)
...
# -------------------------------
# 2. Calcul empirique des transitions 2 → 3
# -------------------------------
phenology_long <- phenology_data_numeric %>%
...
# -------------------------------
# 3. Créer la matrice 5x5 (strates j, nf, f, ff, F)
# -------------------------------
states <- c("1","2","3","4","5")  # 5 strates
num_states <- length(states)
Psi_matrix <- matrix(0, nrow=num_states, ncol=num_states, dimnames = list(states, states))

for(i in 1:nrow(transitions)){
  from_state <- transitions$state[i]
  to_state   <- transitions$next_state[i]
  Psi_matrix[from_state, to_state] <- transitions$prob[i]
}
...
for(i in 1:num_states){
  for(j in 1:num_states){
    prob <- Psi_matrix_norm[i,j]
    if(!is.na(prob) && prob >= 0.01){   # seuil d’affichage
      dot_code <- paste0(dot_code, states[i], " -> ", states[j], 
                         " [label=\"", round(prob,2), "\"];\n")
    }
  }
...
phenology_data_numeric <- data %>%
  ...
  filter(!is.na(phenoS2) & !is.na(phenoS3))  # supprimer NA

# -------------------------------
# 2. Calcul empirique des transitions
# -------------------------------
phenology_long <- phenology_data_numeric %>%
  pivot_longer(cols = c(phenoS2, phenoS3),
               names_to = "session",
...
# Créer la matrice 5x5 pour les strates existantes + F
states <- c("1","2","3","4","5")
num_states <- length(states)
Psi_matrix <- matrix(0, nrow=num_states, ncol=num_states, dimnames = list(states, states))
...
# -------------------------------
# 3. Visualisation avec DiagrammeR
# -------------------------------
dot_code <- "digraph pheno_transitions { 
  rankdir=LR;
  node [shape=circle, style=filled, color=lightblue, fontname=Helvetica];
...
for(i in 1:num_states){
  for(j in 1:num_states){
    prob <- Psi_matrix[i,j]
    if(!is.na(prob) && prob >= 0.01){   # seuil affichage
      dot_code <- paste0(dot_code, states[i], " -> ", states[j], 
                         " [label=\"", round(prob,2), "\"];\n")
    }
  }
...
# -------------------------------
# 4. Partie RMark pour phenoS2 → phenoS3
# -------------------------------
# Créer le champ "ch" pour MARK
phenology_data_numeric <- phenology_data_numeric %>%
...
ms_data <- process.data(
  phenology_data_numeric,
  model = "Multistrata",
  strata.labels = c("1","2","3","4","5")  # inclure F
)
...
# Résultats MARK
df <- ms_model$results$real %>%
  rownames_to_column(var = "full_name") %>%
  separate(full_name,
           into = c("param","from","to","group","c","a","o","t"),
           sep = " ",
           fill = "right") %>%
...
# Filtrer les transitions Psi et nettoyer
df_psi <- df %>%
  filter(param == "Psi") %>%
  mutate(
    from = str_trim(str_extract(from, "\\d+")),
    to   = str_trim(str_extract(to, "\\d+"))
  ) %>%
  filter(!is.na(from) & !is.na(to))
...
# Graphe DiagrammeR (afficher seulement transitions >= 0.01)
dot_code <- "digraph pheno_transitions { 
  rankdir=LR;
  node [shape=circle, style=filled, color=lightblue, fontname=Helvetica];
"

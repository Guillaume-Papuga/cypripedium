...
Pour éviter la colinéarité entre les stades phénologiques et la taille, deux modèles sont effectués en mettant à chaque fois une des deux variables. Les modèles sont des GLMM binomiaux avec comme variable réponse détection. Les variables explicatives fixes sont : - le site, la session et leur interaction : On s'attend à ce que la détectabilité augmente avec le temps, quelle varie potentiellement entre sites et que cet détectabilité entre sites change au cours du temps - la phénologie observée par les opérateurs : On s'attend à ce que le stade juvénile soit détectable moins facilement et à des erreurs entre fleurs fannées et non fleuri - les strates végétales : on s'attend à ce que la détectabilité soit moins bonnes dans des zones denses - le risque d'erreur basé sur l'autonotation des observateurs : on s'attend à ce que la détectabilité diminue avec le risque d'erreur Les variables explicatives aléatoires sont : - l'individu emboité dans quadra (déjà intégré dans notre écriture de l'individu) - le quadra emboité dans site (déjà intégré dans notre écriture du quadrat) - l'observateur
...
# Avant on tente CRDMS des transitions

#VII_i.i Toutes transitions

```{r}
# 1. Fixer p=1 pour toutes les strates (détection parfaite)
crdms.ddl$p$fix <- 1

# 2. Survie constante (ou on peut la fixer à 1 si on ne veut pas l'estimer)
S.list <- list(formula = ~1)

# 3. Détection constante (fixée à 1)
p.list <- list(formula = ~1)

# 4. Probabilités de transition Psi dépendant seulement de l'état courant
#    ici ~stratum = chaque état peut aller vers n'importe quel autre
Psi.list <- list(formula = ~stratum)

# 5. Estimer le modèle
model.simple <- mark(
  crdms.data,
  ddl = crdms.ddl,
  model.parameters = list(S = S.list, p = p.list, Psi = Psi.list),
  threads = 2,
  delete = TRUE
)

# 6. Résultats des probabilités de transition
model.simple$results$real
```

...
1_ Pour l’interprétation, les tests de significativité reposent généralement sur la p‑value. Les seuils couramment utilisés sont :
p < 0.001 : très hautement significatif ; p < 0.01 : hautement significatif ; p < 0.05 : significatif ; p < 0.1 : tendance ; p ≥ 0.1 : non significatif.

2_ Le chargement complet des packages est indispensable. Si un package n’est pas reconnu, il peut être installé avec :
install.packages("nom_du_package")

3_ Si un élément du script ne fonctionne pas, vérifiez que la structure des vecteurs correspond bien au format attendu. De même, assurez‑vous que les fichiers de données sont correctement formatés avant leur importation.

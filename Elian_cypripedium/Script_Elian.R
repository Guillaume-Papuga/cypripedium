...
phenology_data_numeric <- data %>%
  select(ID_ind, phenoS1, phenoS2, phenoS3) %>%
  mutate(
...
# -------------------------------
# 4. Préparer les données pour RMark
# -------------------------------
...
# -------------------------------
# 5. Modèle Multistrata avec RMark
# -------------------------------
ms_model <- mark(
  ms_data,
  ms_ddl,
...
# -------------------------------
# 6. Calcul empirique des transitions 1 → 2 → 3
# -------------------------------
...
# -------------------------------
# 7. Visualisation avec DiagrammeR
# -------------------------------
...
phenology_data_numeric <- data %>%
  select(ID_ind, phenoS1, phenoS2, phenoS3) %>%
  # Supprimer les lignes avec NA dans phénoS1 ou phénoS2 ou phénoS3
  filter(!is.na(phenoS1) & !is.na(phenoS2) & !is.na(phenoS3)) %>%
  mutate(
    phenoS1 = phenology_codes[phenoS1],
    phenoS2 = phenology_codes[phenoS2],
...
# -------------------------------
phenology_data_numeric <- phenology_data_numeric %>%
  mutate(ch = paste0(phenoS1, phenoS2, phenoS3))  # longueur constante
...
ms_model <- mark(
  ...
  ms_ddl,
  model.parameters = list(
    Psi = list(formula = ~stratum),  # transitions selon état actuel
    p = list(formula = ~1)           # probabilité de capture constante
  )
)
...
# -------------------------------
# 6. Calcul empirique des transitions session 1 → 2
# -------------------------------
phenology_s1_s2 <- phenology_data_numeric %>%
  select(ID_ind, phenoS1, phenoS2)  # seulement sessions 1 et 2

# Calcul des transitions
transitions <- phenology_s1_s2 %>%
  filter(phenoS1 != "" & phenoS2 != "") %>%   # suppression NA éventuels
  count(phenoS1, phenoS2) %>%
  group_by(phenoS1) %>%
  mutate(prob = n / sum(n))

# Créer matrice de transition 5x5
num_states <- 5
Psi_matrix <- matrix(0, nrow = num_states, ncol = num_states)
states <- c("j","nf","f","ff","F")
rownames(Psi_matrix) <- colnames(Psi_matrix) <- states

for(i in 1:num_states){
  for(j in 1:num_states){
    val <- transitions %>% filter(phenoS1 == i, phenoS2 == j) %>% pull(prob)
    if(length(val) > 0) Psi_matrix[i,j] <- mean(val)
  }
}
...
# -------------------------------
# 7. Visualisation des transitions avec DiagrammeR
# -------------------------------
dot_code <- "digraph pheno_transitions { 
  rankdir=LR;
  node [shape=circle, style=filled, color=lightblue, fontname=Helvetica];
...
phenology_data_numeric <- data %>%
  select(ID_ind, phenoS1, phenoS2, phenoS3) %>%
  # Supprimer les lignes avec NA dans phénoS2 ou phénoS3 (session 2→3)
...
# -------------------------------
# 6. Calcul empirique des transitions session 2 → 3
# -------------------------------
phenology_s2_s3 <- phenology_data_numeric %>%
  select(ID_ind, phenoS2, phenoS3)

# Calcul des transitions
transitions <- phenology_s2_s3 %>%
  filter(phenoS2 != "" & phenoS3 != "") %>%  # supprimer NA éventuels
  count(phenoS2, phenoS3) %>%
  group_by(phenoS2) %>%
  mutate(prob = n / sum(n))
...
for(i in 1:num_states){
  for(j in 1:num_states){
    val <- transitions %>% filter(phenoS2 == i, phenoS3 == j) %>% pull(prob)
    if(length(val) > 0) Psi_matrix[i,j] <- mean(val)
  }
}

...
::: {style="border: 2px solid #8B0000;   border-radius: 10px;   padding: 10px;   background-color: #FDECEA;   font-family: Arial, sans-serif;   line-height: 1.2;   color: #333;   margin-bottom: 10px;"}
...
Le modèle CRDMS sur Rmark n'accepte pas d'avoir des histoires de capture de taille différents or en fonction des quadrats, il y a un nombre d'observateurs différents. On a donc choisit d'homogénéiser le nombre d'observateurs à 5 en elevant les quadrats avec moins d'observateurs (3 quadrats) et en tirant les 5 observateurs avec le plus de quadrats observés quand il y avait 6 observateurs dans un quadrat. Note : Tête cendrée bas a été éliminé dans le processus d'homogénéisation à 5 observateurs par quadrat et session minimum. Il y aura donc l'analyse de 4 sites seulement.

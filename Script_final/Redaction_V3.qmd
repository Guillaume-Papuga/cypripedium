...
<div style="
  ...
  margin-bottom: 10px;              
">
  <strong>Travail collaboratif :</strong> Ce travail a été réalisé dans le cadre de l'enseignement du <strong>Master 2 RAINET</strong> de l'Université de Montpellier.
  <br>
  <strong>Étudiants :</strong> Galadriel Fourtier et Elian Vuillemot
  <br>
  <strong>En collaboration avec :</strong> Parc national de Forêts, Guillaume Papuga et Thibaut Couturier
  <br>
  <strong>Contact :</strong> Pour toute question, nous sommes disponibles aux emails suivants : 
  <a href="mailto:vuillemotelian@gmail.com">vuillemotelian@gmail.com</a>, 
  <a href="mailto:galadriel.fourtier@gmail.com">galadriel.fourtier@gmail.com</a>
</div>
...
verification <- read.csv(
  "data/raw/detections_individuelles_verifsTC+LD_18dec.csv",
  sep = ";"
)
...
data_original <- read.csv(
  "data/raw/Taille et phéno verif LD.csv",
  sep = ";"
)
...
# Lire le fichier CSV
experience <- read.csv(
  "data/raw/Experience_des_observateurs_VF.csv",
  sep = ";"
)
...
Une méthode pour intégrer le recouvrement serait de mettre en place un indice basé sur les strates et leur hauteur. En fixant une valeur seuil de hauteur pour chaque strate, on peut quantifier et pondérer les hauteurs moyennes par quadrat. Cette méthode reste à développer.
Ici un indice de recouvrement des strates ligneuses a été mis en place. On prends la médiane de la taille pour les strates arbustives et la strate arborescente (soit 2m, 5m et 10m) qu'on multiplie par le recouvrement à chaque strate. Puis on divise par la somme des recouvrements des trois strates. On obtiens ainsi une moyenne de la hauteur des ligneux sur le quadrat.
...
**Résultats** : Le graphique montre que les observateurs qui ont du mal à attribuer correctement les stades phénologiques ont aussi tendance à manquer davantage d’événements.
Cela peut indiquer : une difficulté générale avec l’identification, un effet d’expérience (moins d’habitude → plus d’erreurs), ou des différences individuelles dans la rigueur ou la méthode.
...
l’*individu*, emboîté dans le *quadrat* (déjà pris en compte dans la structure d’identifiant) ;
le *quadrat*, emboîté dans le *site* ;
l’*observateur*, afin de tenir compte des variations inter‑individuelles de compétence et de style d’observation.
...
### Préparation tableau
Ajouter explication d'homogénéisation à 5 observateurs.
Note : pas tete cendrée bas car éliminé dans le processus d'homogénéisation à 5 observateurs par quadrat et session minimum.

...
tab_session <- tab_complet_final %>%
  ...
  mutate(code = phenology_codes[as.character(phenologie)]) %>%
  
  ### 2. ordre numérique propre
  mutate(
    obs_num = dense_rank(observateur)
  ) %>%
...
crdms.data<-process.data(crdms_input,
                         ...
                         groups = "site",
                         time.interval=t.int,
			strata.labels = c("0", "1", "2", "3", "4", "5")
)

crdms.ddl <- make.design.data(crdms.data)

# Créer un index pour fixer p = 0 et c= 0pour les états jamais observés ("0") si besoin

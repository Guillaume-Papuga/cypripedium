...
ggplot(recouvrement, aes(x = Quadrat, y = hauteur_moy))+
  geom_point()+
  facet_wrap(~Site)
...
Nous disposons de plusieurs strates, mais nous souhaitons n’utiliser qu’une seule covariable représentant le recouvrement. Pour cela, nous avons recours à une ACP, qui permet de combiner les variables initiales en un indicateur synthétique basé sur l’inertie des axes 1 et 2. 
...
# -------------------------------------------------------------------
# Tableau kable des coefficients
# -------------------------------------------------------------------

coef_tab <- broom.mixed::tidy(glmm_final_pheno, effects = "fixed")

kable(
  coef_tab,
  caption = "Coefficients du modèle GLMM (effets fixes) – Modèle phénologie",
  digits = 4,
  align = "c"
) %>%
  kable_styling(full_width = FALSE)
...
```
**Méthode d'interprétation** : Le tableau des coefficients indique pour chaque variable si elle augmente ou diminue la probabilité de détection et si son effet est significatif, ce qui permet d’identifier les prédicteurs réellement influents dans le modèle. La vérification de la surdispersion sert à contrôler la validité statistique du GLMM : une surdispersion proche de 1 confirme que le modèle est bien ajusté. Les valeurs de R² marginal et conditionnel renseignent sur la part de variance expliquée par les seules variables fixes, puis par l’ensemble du modèle incluant les effets aléatoires, indiquant ainsi la contribution des différences entre observateurs et entre quadrats. Enfin, les estimations marginales (EMMEANS) permettent de visualiser comment la probabilité de détection varie selon les combinaisons site × session, en isolant l’effet propre de cette interaction et en fournissant des intervalles de confiance qui facilitent l’interprétation des différences entre groupes. 
...
Le modèle montre une bonne qualité d’ajustement : l’absence de surdispersion (test DHARMa) confirme sa validité statistique. Les variables fixes expliquent environ 18 % de la variance totale, tandis que l’inclusion des effets aléatoires (observateur, quadrats et leurs imbrications) porte l’explication à 54 %, ce qui souligne l’importance des effets individuels et spatiaux dans la probabilité de détection. Cela confirme que, au‑delà des caractéristiques mesurées, des différences systématiques entre observateurs et entre quadrats influencent fortement la probabilité de détecter un individu.
...
## Modèle nombre de ramets par quadrat
...
## Modèle nombre de détection par observateur
...
## Modèle densité de couvert végétal
